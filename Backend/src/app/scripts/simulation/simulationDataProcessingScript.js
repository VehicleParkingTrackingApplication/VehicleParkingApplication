...
// Function to get folder name for an area
function getAreaFolderName(area) {
    return `${area.name}_${area._id}`;
}

// Function to parse CSV line manually (since we need to process line by line)
function parseCSVLine(line) {
    const values = line.split(',');
    return {
        date: values[0],
        time: values[1],
        parkingAreaId: values[2],
        plateNumber: values[3],
        country: values[4],
        confidence: values[5],
        angle: values[6],
        image: values[7],
...
export async function processSimulationData(areaId) {
    if (mongoose.connection.readyState === 0) {
        await mongoose.connect(MONGO_URI);
    }
    
    // access the area collection from areaId
    const area = await Area.findById(areaId);
    if (!area) {
    ...
    // Get the simulation folder path for this area
    const areaFolderName = getAreaFolderName(area);
    const simulationPath = path.join(process.cwd(), 'public', 'simulation', areaFolderName);
    
    // Check if the folder exists
    if (!fs.existsSync(simulationPath)) {
        console.error(`Simulation folder not found: ${simulationPath}`);
        return;
    }
    
    // Get all CSV files in the folder
    const files = fs.readdirSync(simulationPath)
        .filter(file => file.toLowerCase().endsWith('.csv'))
        .sort(); // Sort alphabetically (which will be chronological for date-based filenames)
    ...
    for (const file of files) {
        ...
        const fileDate = file.replace('.csv', '');
        ...
        const dataLines = lines[0].includes('date,time,parkingAreaId') ? lines.slice(1) : lines;
        ...
        for (let i = 0; i < dataLines.length; i++) {
            ...
            // Determine if we should process this line
            if (!saveDateObj) {
                // No saved timestamp, process from the beginning
                shouldProcess = true;
            } else {
                // Check if this line is after our saved timestamp
                if (rowDateTime > saveDateObj) {
                    shouldProcess = true;
                } else if (rowDateTime.getTime() === saveDateObj.getTime()) {
                    // Same timestamp, check if we need to resume from a specific line
                    if (!foundResumePoint) {
                        // Find the exact line to resume from
                        const savedTime = saveDateObj.toISOString();
                        const savedTimeStr = savedTime.substring(11, 23); // Get HH:MM:SS.mmm part
                        const rowTimeStr = row.time.replace(/-/g, ':');
                        
                        if (rowTimeStr >= savedTimeStr) {
                            shouldProcess = true;
            ...
            if (shouldProcess) {
                ...
                // Update the latest processed datetime
                if (!latestProcessedDate || rowDateTime > latestProcessedDate) {
                    latestProcessedDate = rowDateTime;
                }
                
                // Update area's savedTimestamp after each line
                area.savedTimestamp = rowDateTime.toISOString();
                await area.save();
                console.log(`Updated area's savedTimestamp to ${area.savedTimestamp} (${formatAustralianTime(rowDateTime)} AEST)`);
            }
        }
    }
    
    console.log(`Finished processing simulation data for area: ${area.name}`);
}

// Function to process all areas
export async function processAllSimulationData() {
    if (mongoose.connection.readyState === 0) {
        await mongoose.connect(MONGO_URI);
    }
    ...
    try {
        ...
        for (const area of areas) {
            console.log(`\nProcessing area: ${area.name} (${area._id})`);
            await processSimulationData(area._id);
        }
        
        console.log('\nFinished processing all areas');
    } catch (error) {
        console.error('Error processing all simulation data:', error);
    }
}
